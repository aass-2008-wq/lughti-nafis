...
function honorPage(){return title('مثابرة · تعلم · تميز','لوحة الشرف','نفخر بإنجازاتكم ونحتفي بتقدمكم.')+'<div id="honor-public" aria-live="polite"><p class="hint">جارٍ تحميل لوحة الشرف…</p></div>';}
async function honorLoadPublic(force=false){
 ...
 try{honorPublic=await rpc({action:'honorPublic'});honorFetched=Date.now();draw();}catch(e){if(e.code==='BAD_ACTION'||e.code==='HONOR_SETUP'){honorPublic={published:false};honorFetched=Date.now();draw();return;}const el=$('#honor-public');if(el)el.innerHTML=location.hash==='#honors'?'<div class="empty"><p>تعذر عرض لوحة الشرف الآن.</p>'+btn('إعادة المحاولة','id="honor-retry"')+'</div>':'';if($('#honor-retry'))$('#honor-retry').onclick=()=>honorLoadPublic(true);}finally{honorPending=false;}
}
function honorAdminPage(){if(!teacherSession)return title('لوحة المعلم','إدارة لوحة الشرف','')+'<a class="btn" href="#teacher">دخول المعلم</a>';return '<a class="quiz-back" href="#teacher">→ العودة إلى التقارير</a>'+title('لوحة المعلم','إدارة لوحة الشرف','تميز في الاختبارات · مثابرة في المشاركة')+'<div id="honor-admin" aria-live="polite"><p>جارٍ تحميل إعدادات اللوحة…</p></div>';}
...
async function honorFindCandidates(event){
 ...
 if(settings.mode==='excellence'&&!settings.examId){$('#honor-error').textContent='اختر الاختبار المطلوب للتكريم.';return;}
...
function honorPreview(settings){
 ...
 const preview={...payload,examTitle:C.exams.find(e=>e.id===settings.examId)?.title||'',entries:selected.map(s=>({...s,metric:settings.mode==='participation'?s.count:s.percent}))};
 openDialog('<h2>معاينة لوحة الشرف</h2>'+honorDisplay(preview)+'<p>ستظهر هذه الأسماء لجميع زوار الموقع.</p><p id="honor-publish-error" class="error" role="alert"></p><div class="actions">'+btn('اعتماد ونشر','id="honor-publish"')+btn('العودة للتعديل','id="honor-cancel"','light')+'</div>');
